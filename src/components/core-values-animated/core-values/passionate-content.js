...
class PassionateContent extends Component {
        ...
        return (
            data.allWordpressWpCoreValue.edges.map(post => (
                <ValueContainer>
                    ...
                    <SvgBox>
                        <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 39.688 39.688" class="p-rotate">
                    ...
                    <ValueToggle id={"p_scroll"} className="p-toggle" onClick={() => this.toggleValue()} ><h3>{post.node.title}</h3><FaChevronDown size={24} className={valueName}/></ValueToggle>
...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWordpressWpCoreValue(filter: {categories: {elemMatch: {wordpress_id: {eq: 31}}}}) {
                edges {
                    node {
                        title
                        content
      ...
      `}
      render={data => <PassionateContent data={data} {...props} />}
    />
  );