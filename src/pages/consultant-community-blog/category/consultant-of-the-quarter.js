...
const ConsultantOfTheQuarterPage = ({ data }) => (
    <CCLayout>
        ...
        <CCmain>
            <TitleSection>
                ...
                <hr/>
                <h1>Consultant of the Quarter</h1>
            </TitleSection>
            <CCrow>
                {data.allWordpressWpCcBlogPost.edges.map(post => (
                    <CategoryCol>
                        <article>
                            <Link
                                to={`/consultant-community-blog/${post.node.slug}`}
                            >
                                <ImageContainer>
                                    <Img sizes={post.node.acf.consultant_image.localFile.childImageSharp.sizes} alt={post.node.title} />
...
export default ConsultantOfTheQuarterPage

export const query = graphql`
    query {
        allWordpressWpCcBlogPost(filter: {categories: {elemMatch: {wordpress_id: {eq: 53}}}}, sort: {fields: date, order: DESC}) {
            edges {
                node {
                title
                slug
