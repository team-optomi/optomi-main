...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWordpressWpCustomPage(filter: {categories: {elemMatch: {wordpress_id: {eq: 66}}}}) {
                edges {
                    node {
                        featured_media {
                            localFile {
            ...
            allWordpressWpGbBlogPost(filter: {categories: {elemMatch: {wordpress_id: {in: 66}}}}) {
                edges {
                    node {
                        title
                        slug
      ...
      `}
      render={data => <Corporate data={data} {...props} />}
    />
  );