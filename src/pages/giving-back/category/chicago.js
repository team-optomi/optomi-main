...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWordpressWpCustomPage(filter: {categories: {elemMatch: {wordpress_id: {eq: 64}}}}) {
                edges {
                    node {
                        featured_media {
                            localFile {
            ...
            allWordpressWpGbBlogPost(filter: {categories: {elemMatch: {wordpress_id: {in: 64}}}}) {
                edges {
                    node {
                        title
                        slug
      ...
      `}
      render={data => <Chicago data={data} {...props} />}
    />
  );