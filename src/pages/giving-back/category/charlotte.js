...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWordpressWpCustomPage(filter: {categories: {elemMatch: {wordpress_id: {eq: 60}}}}) {
                edges {
                    node {
                        featured_media {
                            localFile {
            ...
            allWordpressWpGbBlogPost(filter: {categories: {elemMatch: {wordpress_id: {in: 60}}}}) {
                edges {
                    node {
                        title
                        slug
      ...
      `}
      render={data => <Charlotte data={data} {...props} />}
    />
  );