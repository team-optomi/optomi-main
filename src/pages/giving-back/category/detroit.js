...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWordpressWpCustomPage(filter: {categories: {elemMatch: {wordpress_id: {eq: 62}}}}) {
                edges {
                    node {
                        featured_media {
                            localFile {
            ...
            allWordpressWpGbBlogPost(filter: {categories: {elemMatch: {wordpress_id: {in: 62}}}}) {
                edges {
                    node {
                        title
                        slug
      ...
      `}
      render={data => <Detroit data={data} {...props} />}
    />
  );