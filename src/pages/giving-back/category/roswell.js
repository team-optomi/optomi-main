...
class Roswell extends Component {
...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWordpressWpCustomPage(filter: {categories: {elemMatch: {wordpress_id: {eq: 65}}}}) {
                edges {
                    node {
                        featured_media {
                            localFile {
            ...
            allWordpressWpGbBlogPost(filter: {categories: {elemMatch: {wordpress_id: {in: 65}}}}) {
                edges {
                    node {
                        title
                        slug
      ...
      `}
      render={data => <Roswell data={data} {...props} />}
    />
  );