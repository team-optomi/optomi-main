...
class PressMonthArchiveV2 extends React.Component {
    render() {
        const metas = this.props.data.allWordpressWpCustomPage.edges
        const posts = this.props.data.allWordpressWpPressRelease.edges

        const { pubTitle } = this.props.pageContext

      return (
        <PressLayout>
            {metas.map(meta => (
            <SEO 
                title={`${meta.node.acf.meta_title} - ${pubTitle}`}
...
export default PressMonthArchiveV2

export const query = graphql`
    query pressByMonthV2($pubDate: String) {
        allWordpressWpPressRelease(
            filter: {acf: {published_slug: {eq: $pubDate}}}
